...
import { tags, users, currentUser, getUserInitials, getUserById } from '@/data/mock';
...
export function MemoEditDialog({ memo, open, onOpenChange }: MemoEditDialogProps) {
  ...
  const [referencedMemoIds, setReferencedMemoIds] = useState<string[]>(memo.referencedMemoIds);
  ...
  const toggleRecipient = (userId: string) => {
    setRecipientIds(prev => prev.includes(userId) ? prev.filter(id => id !== userId) : [...prev, userId]);
  };
  ...
  const handleSave = () => {
    editMemo(memo.id, { title, body, tags: selectedTags, visibility, attachments, recipientIds, referencedMemoIds }, currentUser.id);
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            {recipientIds.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mb-2">
                {recipientIds.map(uid => {
                  ...
                  return user ? (
                    <Badge key={uid} variant="default" className="gap-1 cursor-pointer" onClick={() => toggleRecipient(uid)}>
            ...
            <div className="flex flex-wrap gap-2 max-h-24 overflow-auto">
              {filteredUsers.filter(u => !recipientIds.includes(u.id)).map(user => (
                <Badge key={user.id} variant="outline" className="cursor-pointer gap-1.5" onClick={() => toggleRecipient(user.id)}>
                  <Avatar className="h-4 w-4">
          ...
          <div>
            ...
            <div className="flex gap-2">
              <Input placeholder="Add custom tag..." value={customTagInput} onChange={e => setCustomTagInput(e.target.value)}
                className="h-8 text-sm w-48" onKeyDown={e => e.key === 'Enter' && (e.preventDefault(), addCustomTag())} />
              <Button type="button" variant="outline" size="sm" className="h-8" onClick={addCustomTag} disabled={!customTagInput.trim()}>Add</Button>
            </div>
          </div>
