...
const Groups = () => {
  ...
  const canCreateGroups = hasPermission("canCreateGroups");
  ...
  const getGroupUnreadCount = (group: Group) => {
    ...
    return getUnreadCount(groupConversation.id, currentUser.id);
  };
    ...
    <AppLayout title="Groups & Departments">
      <div className="max-w-4xl mx-auto space-y-6">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <Badge
                          variant="secondary"
                          className={`text-[10px] ${typeColors[group.type] || ""}`}
                        >
                          {group.type}
                        </Badge>
                      </div>
        ...
        {invitedGroups.length > 0 && (
          <div>
            <h3 className="font-display font-semibold text-sm mb-3 text-warning">
              Pending Invitations
            </h3>
            <div className="grid md:grid-cols-2 gap-4">
              {invitedGroups.map((group) => (
                <div
                  key={group.id}
                  className="widget-card cursor-pointer hover:shadow-md transition-shadow border-warning/30 border"
                  onClick={() => handleGroupClick(group)}
                >
                  <div className="flex items-start gap-3">
        ...
        {otherGroups.length > 0 && (
          <div>
            <h3 className="font-display font-semibold text-sm mb-3 text-muted-foreground">
              Other Groups
            </h3>
            <div className="grid md:grid-cols-2 gap-4">
              {otherGroups.map((group) => (
                <div
                  key={group.id}
                  className="widget-card opacity-60 cursor-not-allowed"
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground mt-2">
                        {group.memberIds.length} members · Not a member
                      </p>
                    </div>
                  </div>
                </div>
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Select
                value={newType}
                onValueChange={(v) => setNewType(v as Group["type"])}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
              ...
              <div className="flex flex-wrap gap-2">
                {otherUsers.map((user) => (
                  <Badge
                    key={user.id}
                    variant={
                      newMembers.includes(user.id) ? "default" : "outline"
                    }
