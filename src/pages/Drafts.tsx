...
const Drafts = () => {
  ...
  const drafts = memos.filter(m => m.status === 'draft' && m.creatorId === currentUser.id);

  return (
    <AppLayout title="My Drafts">
      <div className="max-w-4xl mx-auto space-y-4">
        <p className="text-muted-foreground text-sm">{drafts.length} draft{drafts.length !== 1 ? 's' : ''}</p>

        {drafts.length === 0 ? (
          <div className="text-center py-16">
            <FileText className="h-12 w-12 text-muted-foreground/30 mx-auto mb-3" />
            <p className="text-muted-foreground">No drafts yet</p>
            <Button className="mt-4" onClick={() => navigate("/compose")}>Compose a Memo</Button>
          </div>
        ) : (
              ...
              <div key={draft.id} className="widget-card space-y-2">
                ...
                <div className="flex gap-1.5 flex-wrap">
                  ...
                  <Button size="sm" className="gap-1.5 h-8 text-xs"
                    onClick={() => { updateMemo(draft.id, { status: 'sent' }); toast.success("Memo sent!"); }}>
                    <Send className="h-3.5 w-3.5" /> Send
                  </Button>
                  <Button variant="ghost" size="icon" className="h-8 w-8"
                    onClick={() => { deleteMemo(draft.id); toast.success("Draft deleted"); }}>
