...
import { notifications, conversations, groups, currentUser, getUserById } from "@/data/mock";
import {
  FileText, Bell, Users, MessageSquare,
  CheckCircle2, Clock, TrendingUp, Pin,
} from "lucide-react";
...
const Dashboard = () => {
  ...
  const stats = [
    { label: "Total Memos", value: memos.length, icon: FileText, color: "text-primary", bgColor: "bg-primary/10" },
    { label: "Unread Notifications", value: notifications.filter(n => !n.read).length, icon: Bell, color: "text-destructive", bgColor: "bg-destructive/10" },
    { label: "Active Groups", value: groups.filter(g => g.memberIds.includes(currentUser.id)).length, icon: Users, color: "text-accent", bgColor: "bg-accent/10" },
    { label: "Conversations", value: conversations.length, icon: MessageSquare, color: "text-info", bgColor: "bg-info/10" },
  ];
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="widget-card">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="ml-auto text-xs text-muted-foreground">{pendingApprovals.length} items</span>
            </div>
          ...
          <div className="widget-card">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="ml-auto text-xs text-muted-foreground">{pinnedMemos.length} items</span>
            </div>
            <div className="space-y-3">
